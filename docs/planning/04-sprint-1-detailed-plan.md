## Implementation status (2026-09-21) — backend complete, frontend not started
...
**Not started: the Driver Mobile App (React Native) and `packages/ui-native`.** Deliberately deferred — its tooling (Expo, a simulator/device) and testing story are different enough from the two web surfaces that it's a separate, comparably-sized effort, not a quick extension of what's above.

**Still-open items, unchanged from before implementation:** whether the `support` admin role can suspend/restore or should be read-only-plus-masked; the shared component package structure (`design-tokens`/`ui-web`, now built as proposed) still wants Arjun's explicit sign-off, not just tacit adoption; the Sprint 0 Engineering Gate is still not formally checked off in `02-kanban-board.md`.
...
- [ ] **OPEN ITEM — needs the user's product decision, not silently defaulted (added during architecture review):** `vehicles.registration_no` uniqueness scope — per-user (as currently designed, allows the same plate active under two different users) vs. global-while-active. See Risk #6, Step 2. Does not block Sprint 1 implementation; must be resolved before `ACC-03`/`TR-04` (Sprint 5/6) start depending on this table.
