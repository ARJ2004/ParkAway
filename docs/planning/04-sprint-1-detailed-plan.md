...
**Still-open items requiring the user's decision, unchanged by this update:** whether the `support` admin role can suspend/restore or should be read-only-plus-masked; the Sprint 0 Engineering Gate is still not formally checked off in `02-kanban-board.md`; the phone-number-recycling identity risk (explicitly deferred by the user — see the vehicle-registration update below, not silently dropped). See Arjun's sign-off in Step 6 below for the full review of the finished implementation.

## Update (2026-09-22) — vehicle registration decisions + mobile navigation redesign

**Vehicle registration uniqueness is now global** (user decision, resolving the open item above) — one active account per plate system-wide, not per-user. Added proper Indian registration-number format validation (`KA05HR1096`-style) alongside it. **Phone-number recycling was explicitly deferred** by the user ("leave this for now") rather than resolved — still open, not forgotten. Writing this feature's test also caught a real latent bug in `pgConstraintName()` (it never actually reached the wrapped Postgres error's message, so constraint-specific retry logic had been silently dead since it was written) — fixed by reading `constraint_name` directly instead of regex-parsing `.message`. Full detail and verification in the commit (`4411f39`).

**The Driver Mobile App's navigation was redesigned** — the first pass had ported the web app's top nav bar (`AppHeader`) onto mobile as-is, which the user correctly called out as wrong: mobile navigation should be a native bottom tab bar, not a web layout replica. Rebuilt using Mobbin MCP research (real reference screens from Freenow, Lyft, Careem, My BMW, Affirm, PayPal, and others — searched for bottom-tab patterns, home-screen status cards, garage/vehicle-list screens, and profile/account layouts) as actual inspiration, synthesized into ParkAway's own warm palette rather than copied:
- **Bottom tab bar** (`@react-navigation/bottom-tabs`, icons via `@expo/vector-icons`'s Ionicons): Home, Vehicles, Profile — exactly the three tabs this sprint has built, no placeholder tab for Search/Bookings before Sprint 3 lands them.
- **Home** is now a real dashboard (greeting, a "search is coming" highlight card, a default-vehicle status card or add-vehicle CTA) instead of a placeholder text block — pattern drawn from Shell/Waymo's single-highlight-card home layout and BMW's status-card treatment.
- **Vehicles** is a card-based garage list (themed icon tile, plate as the primary line, a `Default` badge) instead of plain bordered text rows — drawn from the BMW/Rivian/Mercedes garage-screen references.
- **Profile** now has an avatar-initials header, grouped editable fields, and sign-out as its own clearly separated destructive row at the bottom — instead of a button buried in a top nav bar — drawn from the Affirm/PayPal/Freenow profile-screen references.
- `AppHeader.tsx` (the ported top nav) is deleted; navigation between Home/Vehicles/Profile is now the tab bar itself.
- Added a `Badge` component to `ui-native` (Default/Verified pills) — the first `ui-native` component ui-web doesn't yet have an equivalent extraction for (ui-web still inlines the same concept as ad hoc spans); noted as a small follow-up to keep the two packages' inventories in sync, not urgent.
- Fixed `Card`'s `style` prop type in `ui-native` from `ViewStyle` to `StyleProp<ViewStyle>` (a real bug — the narrower type rejected the array-of-styles pattern used throughout, e.g. `style={[styles.card, {...}]}`) while building the new screens.

**The `.js`-import-extension mistake happened twice more** while building this (in the new `Badge.tsx`, and two pre-existing but previously-unnoticed instances in the web apps' test files — harmless under Vitest, which is why they'd never surfaced). Rather than keep manually re-sweeping the codebase after each recurrence, added a `no-restricted-syntax` ESLint rule (`noJsExtensionOnRelativeImportsRules` in `eslint.config.js`) that flags a `.js`-suffixed relative import/export on every non-backend surface. Verified the rule actually fires (deliberately introduced and confirmed a warning, not just assumed) before relying on it.

**Verified:** typecheck clean, full Metro production bundle succeeds, `npm run lint` clean across the whole monorepo including the new rule, and both web test suites (7 tests) still pass after the extension cleanup. **Still not verified: an actual device/emulator run** — no browser or mobile testing tool was available this session, so the redesign has been checked structurally (typecheck + bundle) but never actually seen or tapped through on a phone.
